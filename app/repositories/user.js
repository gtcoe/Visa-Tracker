...
module.exports = () => {
    const insertUser = async(data) => {
        try{
            ...
            let params = [data.name, data.phone, data.email, data.password, data.address, data.poc, constants.STATUS.USER.ACTIVE, data.last_updated_by, data.type]
    ...
    //done
    const updateStatusAndPassword = async(status, password, userId, lastUpdatedBy) => {
        try{
            let query = `update ${constants.TABLES.USER} set status = ?, password = ?, lastUpdatedBy = ?
            where id = ?`
    ...
    const updateUserLastLoginAt = async (userId) => {
        ...
        try{
            let query = `update ${constants.TABLES.USER} set last_login_at = ${moment(new Date()).format('YYYY-MM-DD HH:mm:ss')} where id = ?`
            ...
            logger.error(`Error in updateUserLastLoginAt : ${utility.generateError(e)}`)
            throw e
        }
    }
