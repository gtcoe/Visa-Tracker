import mysql, { Pool, PoolConnection, QueryError } from "mysql";
import { logger } from "../logging";
import dbConfig from "../config/mysqlConfig";
...
const mysqlPool: Pool = mysql.createPool({
  ...
  idleTimeoutMillis: envDBConfig.idle,
...
class Mysql {
  ...
  protected generateErrorResponse(err: QueryError) {
    ...
    logger.error(`MySQL Error - ${JSON.stringify(result)} | SQL: ${err.sql}`);
    return result;
  }

  async query<T = any>(sql: string, params: any[] = []): Promise<{ status: boolean; data: T | null }> {
